...
const ImageUploader = ({
  ...
  return (
    <div className="mb-5 w-full">
      ...
      <div
        className={`dark:bg-white text-sm font-normal rounded-lg block w-full p-2.5 ${getDivClassName()}`}
      >
        {/* Image Preview */}
        <div className="w-full flex overflow-x-auto mt-4 space-x-2 flex-nowrap justify-center items-center">
          {FIELDS.map((field, index) => {
            const file = RENDER(field);
            console.log("🚀 ~ {FIELDS.map ~ file:", file);
            return file.url ? (
              <div
                key={index}
